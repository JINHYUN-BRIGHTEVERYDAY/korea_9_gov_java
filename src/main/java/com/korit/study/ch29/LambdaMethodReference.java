...
// 람다 메서드 참조
// 람다식을 간결하게 표현하는 방법 (:: 더블콜론 사용)
// 호출구조가 기존의 메서드와 완전히 동일하면 가능
// (매개변수) -> 메서드 (매개변수)
...
//        Supplier<List<String>> listSupplier = ArrayList::new;
        ...
        strNums.stream()
//                .map(str -> Integer.parseInt(str)) -> 가능
                .map(Integer::parseInt)
                .collect(Collectors.toList())
//                .forEach(n -> atomicSumResult.set(atomicSumResult.get() + n)); -> 불가능
                .forEach(atomicSumResult::set);
//        .forEach(n -> atomicSumResult.set(n)); -> 가능
